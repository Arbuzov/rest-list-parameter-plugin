...
image:https://github.com/jenkinsci/rest-list-parameter-plugin/actions/workflows/cd.yaml/badge.svg?branch=main[link="https://github.com/jenkinsci/rest-list-parameter-plugin/actions/workflows/cd.yaml"]
...
** Decorate the displayed value via a regex/replacement pair (e.g. trim a prefix, append a suffix, or reformat with capture groups)
...
[#displayDecoration]
==== Display Decoration (regex transform)

The optional `Display Decoration Pattern` and `Display Decoration Replacement` fields in the Advanced section apply a Java regex `replaceAll` to every <em>display value</em> after it is parsed.
The underlying value passed into the pipeline is *never modified* &mdash; only what the user sees in the dropdown.
Use it to trim a prefix, append a suffix, or reformat with capture groups, without touching the upstream REST response or the `Value Expression`.

image::.media/configuration-display_decoration.png[Display Decoration configuration (Display Expression + Pattern + Replacement)]
...
|Strip leading `v` (`v10.7.7` &rarr; `10.7.7`)
|`^v`
|_(empty)_

|Append a literal suffix (`10.7.7` &rarr; `10.7.7 (stable)`)
|`$`
|` (stable)`

|Reformat with a capture group (`v10.7.7-beta` &rarr; `Release 10.7.7`)
|`^v(\d+\.\d+\.\d+).*`
|`Release $1`
|===

When the pattern is empty the display value is left untouched.
An invalid regex is logged as a warning and the original display value is shown.

image::.media/trigger-build_dropdown_decorated.png[Decorated dropdown — the option text reads "Release 10.11.8" while the option value remains "v10.11.8"]

[IMPORTANT]
====
**Triggering this job from another pipeline (e.g. `build job: ...`) must pass the raw value from the list, not the decorated label the user sees.**
The decoration is purely cosmetic for the dropdown; parameter validation always runs against the raw values returned by the REST endpoint. Passing the decorated string fails with `IllegalArgumentException: Illegal value for parameter ...`.

[source,groovy]
----
// upstream value list contains `v10.11.8`, displayed as `Release 10.11.8`

// ✅ correct — pass the raw value
build job: 'downstream',
      parameters: [string(name: 'VERSION', value: 'v10.11.8')]

// ❌ wrong — the decorated label is not a valid parameter value
build job: 'downstream',
      parameters: [string(name: 'VERSION', value: 'Release 10.11.8')]
----
====
...
pipeline {
  ...
  parameters {
    RESTList(
      ...
      displayDecorationPattern: '^v(\\d+\\.\\d+\\.\\d+).*', // optional
      displayDecorationReplacement: 'Release $1'     // optional
    )
  }
